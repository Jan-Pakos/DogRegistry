...
public class OwnerSorter {
    private static void swapDogs(ArrayList<Owner> ownerArrayList, int indexOne, int indexTwo){
    ...
    public static int sortOwners(Comparator<Owner> comparator, ArrayList<Owner> owners){
        ...
        for(int i = 0; i<owners.size();i++){
            int indexOfDogToSwap = OwnerSorter.nextOwner(comparator, owners, i);
            if(indexOfDogToSwap != i){
                swapDogs(owners, i, indexOfDogToSwap);
                swapsMade++;
